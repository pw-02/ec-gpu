...
impl<'a, E: Engine + GpuEngine> SingleFftKernel<'a, E> {
    ...
    pub fn radix_fft(&mut self, input: &mut [<E as Engine>::Fr], omega: &<E as Engine>::Fr, log_n: u32) -> EcResult<()> {
        let closures = program_closures!(|program, input: &mut [<E as Engine>::Fr]| -> EcResult<()> {
            let n = 1 << log_n;
            // All usages are safe as the buffers are initialized from either the host or the GPU
            // before they are read.
            let mut src_buffer = unsafe { program.create_buffer::<<E as Engine>::Fr>(n)? };
            let mut dst_buffer = unsafe { program.create_buffer::<<E as Engine>::Fr>(n)? };
            // The precalculated values pq` and `omegas` are valid for radix degrees up to `max_deg`
            let max_deg = cmp::min(MAX_LOG2_RADIX, log_n);

            // Precalculate:
            // [omega^(0/(2^(deg-1))), omega^(1/(2^(deg-1))), ..., omega^((2^(deg-1)-1)/(2^(deg-1)))]
            let mut pq = vec![<E as Engine>::Fr::ZERO; 1 << max_deg >> 1];
            let twiddle = omega.pow_vartime([(n >> max_deg) as u64]);
            pq[0] = <E as Engine>::Fr::ONE;
            if max_deg > 1 {
                pq[1] = twiddle;
            ...
            let mut omegas = vec![<E as Engine>::Fr::ZERO; 32];
            omegas[0] = *omega;
    ...
    E: Engine + GpuEngine,
{
    ...
    pub fn radix_fft_many(
        &mut self,
        inputs: &mut [&mut [<E as Engine>::Fr]],
        omegas: &[<E as Engine>::Fr],
        log_ns: &[u32],
    ) -> EcResult<()> {
        let n = inputs.len();
