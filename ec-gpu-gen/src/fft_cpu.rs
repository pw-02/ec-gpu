...
pub fn serial_fft<E: Engine>(a: &mut [<E as Engine>::Fr], omega: &<E as Engine>::Fr, log_n: u32) {
...
pub fn parallel_fft<E: Engine>(
    a: &mut [<E as Engine>::Fr],
    worker: &Worker,
    omega: &<E as Engine>::Fr,
    log_n: u32,
    log_threads: u32,
) {
    ...
    let mut tmp = vec![vec![<E as Engine>::Fr::ZERO; 1 << log_new_n]; num_threads];
    ...
    worker.scope(0, |scope, _| {
        ...
        for (j, tmp) in tmp.iter_mut().enumerate() {
            scope.execute(move || {
                ...
                let mut elt = <E as Engine>::Fr::ONE;
        ...
        fn test_consistency<E: Engine, R: RngCore>(rng: &mut R) {
            ...
            for _ in 0..5 {
                for log_d in 0..10 {
                    ...
                    let mut v1_coeffs =
                        (0..d).map(|_| <E as Engine>::Fr::random(&mut *rng)).collect::<Vec<_>>();
